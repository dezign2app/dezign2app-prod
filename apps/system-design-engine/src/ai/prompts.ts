...
Ensure WebClient events are connected to Service endpoints, and Service endpoints are connected to Database references (db_ref).`;
  ...
  return `You are an expert AI software architect and UI designer.
    ...
    VERY IMPORTANT: DO NOT add nodes that already exist in the Current Canvas State. Carefully review the Current Canvas State before calling add_node. If a component (e.g., a database, a service, a web page) is already present on the canvas, do not create it again. Only add missing components.
    ...
    - WebPage to Service: connects events to endpoints.
