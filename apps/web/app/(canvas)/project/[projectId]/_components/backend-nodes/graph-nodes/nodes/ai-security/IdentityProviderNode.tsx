...
const ProviderRow = ({
  ...
  const actualPresetKey =
    Object.entries(IDENTITY_PROVIDER_PRESETS).find(
      ([_, v]) => v.provider === item.provider,
    )?.[0] || "custom";
  const preset =
    IDENTITY_PROVIDER_PRESETS[
      actualPresetKey as keyof typeof IDENTITY_PROVIDER_PRESETS
    ];
  const capabilities = isCustom
    ? item.customCapabilities || preset?.capabilities || {}
    : preset?.capabilities || {};
  const outputs = isCustom
    ? item.customOutputs || preset?.outputs || {}
    : preset?.outputs || {};
    ...
    >
      <div className="absolute -right-1 top-0 bottom-0 flex flex-col justify-center gap-1.5 py-1 z-10">
        <Handle
          ...
          id={`out-${item.id}`}
          className="w-1.5 h-1.5 !relative !transform-none !right-0 !top-0 bg-blue-500 rounded-sm"
          ...
          <div className="flex items-center gap-1 w-full nodrag">
            <ProviderPresetCombobox
              ...
              onValueChange={(label: string | null) => {
                ...
                const preset = key
                  ? IDENTITY_PROVIDER_PRESETS[
                      key as keyof typeof IDENTITY_PROVIDER_PRESETS
                    ]
                  : null;
...
const ProviderList = ({ nodeId, title }: { nodeId: string; title: string }) => {
  const items = useBackendCanvasStore((s) => s.identityProviders).filter(
    (p) => p.nodeId === nodeId,
  );
  const addIdentityProvider = useBackendCanvasStore(
    (s) => s.addIdentityProvider,
  );
  const updateIdentityProvider = useBackendCanvasStore(
    (s) => s.updateIdentityProvider,
  );
  const deleteIdentityProvider = useBackendCanvasStore(
    (s) => s.deleteIdentityProvider,
  );
  ...
  return (
    <>
      ...
      {items.length === 0 && !editingId ? (
        <div
          className="px-3 py-4 text-xs text-muted-foreground text-center bg-secondary/10 flex items-center justify-center gap-2 cursor-pointer hover:bg-secondary/20 transition-colors"
          onClick={handleAdd}
        >
          <Plus size={12} />
          ...
          {items.map((item) => (
            <ProviderRow
              ...
              handleUpdate={(id, updates) =>
                updateIdentityProvider(id, updates)
              }
...
export const IdentityProviderNode = ({
  id,
  data,
  selected,
  ...
  return (
    <div
      className={cn(
        "shadow-md rounded-xl bg-card border-2 min-w-[280px] max-w-[350px] flex flex-col",
