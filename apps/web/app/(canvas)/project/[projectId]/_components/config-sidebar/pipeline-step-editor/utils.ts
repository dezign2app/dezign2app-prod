...
export function isAutoGeneratedStepName(name?: string): boolean {
  ...
  return /^(condition|tryCatch|switch|parallel|loop|earlyReturn|transformedData|dbResult|cachedResult|publishResult|serviceResponse|externalResult|step)\d+(Result|Results)?$/i.test(
    name.trim(),
  );
}
...
export function formatSourceShort(source?: any): string {
  ...
  if (source.kind === "literal") {
    return typeof source.value === "string" ? `"${source.value}"` : String(source.value ?? "");
  }
  const prefix =
