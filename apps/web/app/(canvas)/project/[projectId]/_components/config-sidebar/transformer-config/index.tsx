...
import { isAutoGeneratedStepName } from "../pipeline-step-editor/utils";
...
export const TransformerConfig: React.FC<TransformerConfigProps> = ({
  ...
  const updateData = React.useCallback(
    (patch: Partial<typeof data>) => {
      ...
      // Synchronize changes to any pipeline steps referencing this transformer
      if (patch.returnSchema !== undefined || patch.functionName !== undefined) {
        const nextReturnSchema = patch.returnSchema ?? currentNode.data?.returnSchema;
        const nextFnName = patch.functionName ?? currentNode.data?.functionName ?? currentNode.data?.label;

        store.endpoints.forEach((ep) => {
          if (!ep.pipelineSteps || ep.pipelineSteps.length === 0) return;
          let changed = false;
          const updatedSteps = ep.pipelineSteps.map((step) => {
            if (step.type !== "transform") return step;
            const matchesThisNode =
              step.transformerNodeId === node.id ||
              step.functionRef?.name === currentNode.data?.functionName ||
              step.functionRef?.name === currentNode.data?.label ||
              (currentNode.data?.functionName &&
                toVarName(step.functionRef?.name || "") === toVarName(currentNode.data.functionName));

            if (!matchesThisNode) return step;

            changed = true;
            const stepUpdates: any = {};
            if (Array.isArray(nextReturnSchema) && nextReturnSchema.length > 0) {
              stepUpdates.outputSchema = nextReturnSchema.map((r: any) => ({
                name: r.name,
                type: r.type || "string",
                required: r.required,
              }));
              if (step.functionRef) {
                stepUpdates.functionRef = {
                  ...step.functionRef,
                  returnSchema: nextReturnSchema,
                  ...(nextFnName ? { name: nextFnName } : {}),
                };
              }
            } else if (nextFnName && step.functionRef) {
              stepUpdates.functionRef = {
                ...step.functionRef,
                name: nextFnName,
              };
            ...
            if (nextFnName && isAutoGeneratedStepName(step.outputVariable)) {
              const newVar = `${toVarName(nextFnName)}Result`;
              stepUpdates.outputVariable = newVar;
              stepUpdates.name = newVar;
            }
