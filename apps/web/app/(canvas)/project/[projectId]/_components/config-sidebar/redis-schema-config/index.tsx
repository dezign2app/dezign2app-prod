...
export const RedisSchemaConfig: React.FC<RedisSchemaConfigProps> = ({
  ...
  const updateData = (changes: Partial<BackendNode["data"]>) => {
    ...
    if (
      (changes.redisDataStructure && changes.redisDataStructure !== data.redisDataStructure) ||
      (changes.jsonRootType && changes.jsonRootType !== data.jsonRootType)
    ) {
      if (data.dbOperations && data.dbOperations.length > 0) {
        const customOps = data.dbOperations.filter((op) => !op.isAutoGenerated);
        const newDefaults = generateRedisOperations(nextData.label || "Cache", nextData);
        nextData.dbOperations = [...newDefaults, ...customOps];
      }
    }
    updateNode(nodeId, {
      data: nextData,
