    ...
    return [
      {
        ...
        signature: `append${pascalSingular}Item<T = Record<string, unknown>>(key: string, item: T): Promise<number>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "item", type: "Record<string, unknown>", required: true },
        ],
        returnType: "Promise<number>",
        logicMode: "code",
        code: `export async function append${pascalSingular}Item<T = Record<string, unknown>>(key: string, item: T): Promise<number> {\n  const redis = await getRedisClient();\n  // Upsert: ensure root array exists\n  await redis.call("JSON.SET", key, "$", "[]", "NX");\n  const res = await redis.call("JSON.ARRAPPEND", key, "$", JSON.stringify(item));\n  return Array.isArray(res) ? ((res[0] as number) || 0) : ((res as number) || 0);\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        ...
        signature: `pop${pascalSingular}Item<T = Record<string, unknown>>(key: string, index: number = -1): Promise<T | null>`,
        params: [
          { name: "key", type: "string", required: true },
        ...
        logicMode: "code",
        code: `export async function pop${pascalSingular}Item<T = Record<string, unknown>>(key: string, index: number = -1): Promise<T | null> {\n  const redis = await getRedisClient();\n  const res = await redis.call("JSON.ARRPOP", key, "$", index);\n  if (!res) return null;\n  try {\n    const parsed = typeof res === "string" ? JSON.parse(res) : res;\n    return (Array.isArray(parsed) ? parsed[0] : parsed) as T;\n  } catch {\n    return null;\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        ...
        signature: `getRecent${pascalSingular}Items<T = Record<string, unknown>>(key: string, count: number = 20): Promise<T[]>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "count", type: "number", required: false, defaultValue: "20" },
        ],
        ...
        logicMode: "code",
        code: `export async function getRecent${pascalSingular}Items<T = Record<string, unknown>>(key: string, count: number = 20): Promise<T[]> {\n  const redis = await getRedisClient();\n  const res = await redis.call("JSON.GET", key, "INDENT", "", "NEWLINE", "", "SPACE", "", "PATH", "$[-" + count + ":]");\n  if (!res) return [];\n  try {\n    const parsed = JSON.parse(res as string);\n    return (Array.isArray(parsed[0]) ? parsed[0] : parsed) as T[];\n  } catch {\n    return [];\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
      ...
      },
      {
        ...
        signature: `get${pascalSingular}<T = Record<string, unknown>[]>(key: string): Promise<T | null>`,
        params: [{ name: "key", type: "string", required: true }],
        returnType: "Promise<Record<string, unknown>[] | null>",
        logicMode: "code",
        code: `export async function get${pascalSingular}<T = Record<string, unknown>[]>(key: string): Promise<T | null> {\n  const redis = await getRedisClient();\n  const res = await redis.call("JSON.GET", key);\n  if (!res) return null;\n  try { return JSON.parse(res as string) as T; } catch { return null; }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        ...
        signature: `set${pascalSingular}<T = Record<string, unknown>[]>(key: string, value: T): Promise<void>`,
        params: [
          { name: "key", type: "string", required: true },
        ...
        logicMode: "code",
        code: `export async function set${pascalSingular}<T = Record<string, unknown>[]>(key: string, value: T): Promise<void> {\n  const redis = await getRedisClient();\n  await redis.call("JSON.SET", key, "$", JSON.stringify(value));\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
...
export function getEntityDbOperations(
  ...
  if (isRedis) {
    if (entityNode.data.dbOperations && entityNode.data.dbOperations.length > 0) {
      ...
      const autoGeneratedOps = entityNode.data.dbOperations.filter((op) => op.isAutoGenerated);
      ...
      return [...redisDefaults, ...customRedisOps];
    }
