...
export function generateRedisOperations(
    ...
    return [
      {
        ...
        signature: `getAll${pascalSingular}Fields(key: string): Promise<GetAll${pascalSingular}FieldsResult>`,
        params: [{ name: "key", type: "string", required: true }],
        returnType: `Promise<GetAll${pascalSingular}FieldsResult>`,
        logicMode: "code",
        code: `export async function getAll${pascalSingular}Fields(key: string): Promise<GetAll${pascalSingular}FieldsResult> {\n  try {\n    const redis = await getRedisClient();\n    const data = await redis.hgetall(key);\n    return { success: true, data: data || {} };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_HGETALL_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        ...
        signature: `get${pascalSingular}Field(key: string, field: string): Promise<Get${pascalSingular}FieldResult>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "field", type: "string", required: true },
        ],
        returnType: `Promise<Get${pascalSingular}FieldResult>`,
        logicMode: "code",
        code: `export async function get${pascalSingular}Field(key: string, field: string): Promise<Get${pascalSingular}FieldResult> {\n  try {\n    const redis = await getRedisClient();\n    const data = await redis.hget(key, field);\n    return { success: true, data };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_HGET_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        ...
        signature: `set${pascalSingular}Fields(key: string, fields: Record<string, string | number>): Promise<Set${pascalSingular}FieldsResult>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "fields", type: "Record<string, string | number>", required: true },
        ],
        returnType: `Promise<Set${pascalSingular}FieldsResult>`,
        logicMode: "code",
        code: `export async function set${pascalSingular}Fields(key: string, fields: Record<string, string | number>): Promise<Set${pascalSingular}FieldsResult> {\n  try {\n    const redis = await getRedisClient();\n    await redis.hset(key, fields);\n    return { success: true, data: undefined };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_HSET_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
      ...
      },
      {
        ...
        signature: `delete${pascalSingular}(key: string): Promise<Delete${pascalSingular}Result>`,
        params: [{ name: "key", type: "string", required: true }],
        returnType: `Promise<Delete${pascalSingular}Result>`,
        logicMode: "code",
        code: `export async function delete${pascalSingular}(key: string): Promise<Delete${pascalSingular}Result> {\n  try {\n    const redis = await getRedisClient();\n    const res = await redis.del(key);\n    return { success: true, data: res > 0 };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_DEL_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
    ];
  ...
  if (structure === "list") {
    return [
      {
        id: `redis-rpush-${tableName}`,
        name: `push${pascalSingular}`,
        kind: "create",
        description: `Append items to Redis list [${label}] (RPUSH)`,
        signature: `push${pascalSingular}(key: string, ...items: string[]): Promise<number>`,
        params: [
          { name: "key", type: "string", required: true },
        ...
        returnType: "Promise<number>",
        logicMode: "code",
        code: `export async function push${pascalSingular}(key: string, ...items: string[]): Promise<number> {\n  const redis = await getRedisClient();\n  return await redis.rpush(key, ...items);\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        id: `redis-lpop-${tableName}`,
        name: `pop${pascalSingular}`,
        kind: "delete",
        description: `Remove and get the first element from Redis list [${label}] (LPOP)`,
        signature: `pop${pascalSingular}(key: string): Promise<string | null>`,
        params: [{ name: "key", type: "string", required: true }],
        returnType: "Promise<string | null>",
        logicMode: "code",
        code: `export async function pop${pascalSingular}(key: string): Promise<string | null> {\n  const redis = await getRedisClient();\n  return await redis.lpop(key);\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        id: `redis-lrange-${tableName}`,
        name: `get${pascalSingular}List`,
        kind: "findAll",
        description: `Get range of items from Redis list [${label}] (LRANGE)`,
        signature: `get${pascalSingular}List(key: string, start: number = 0, stop: number = -1): Promise<string[]>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "start", type: "number", required: false, defaultValue: "0" },
          { name: "stop", type: "number", required: false, defaultValue: "-1" },
        ],
        returnType: "Promise<string[]>",
        logicMode: "code",
        code: `export async function get${pascalSingular}List(key: string, start: number = 0, stop: number = -1): Promise<string[]> {\n  const redis = await getRedisClient();\n  return (await redis.lrange(key, start, stop)) || [];\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        id: `redis-llen-${tableName}`,
        name: `get${pascalSingular}Length`,
        kind: "findById",
        description: `Get length of Redis list [${label}] (LLEN)`,
        signature: `get${pascalSingular}Length(key: string): Promise<number>`,
        params: [{ name: "key", type: "string", required: true }],
        returnType: "Promise<number>",
        logicMode: "code",
        code: `export async function get${pascalSingular}Length(key: string): Promise<number> {\n  const redis = await getRedisClient();\n  return await redis.llen(key);\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        ...
        name: `delete${pascalSingular}`,
        kind: "delete",
        description: `Delete Redis list [${label}] (DEL)`,
  ...
  if (structure === "json" && data?.jsonRootType === "array") {
    const itemType = `${pascalSingular}Item`;
    return [
      {
        id: `redis-json-arrappend-${tableName}`,
        name: `append${pascalSingular}Item`,
        kind: "create",
        description: `Append item to RedisJSON array [${label}] (JSON.ARRAPPEND)`,
        signature: `append${pascalSingular}Item(key: string, item: ${itemType}): Promise<Append${pascalSingular}ItemResult>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "item", type: itemType, required: true },
        ],
        returnType: `Promise<Append${pascalSingular}ItemResult>`,
        logicMode: "code",
        code: `export async function append${pascalSingular}Item(key: string, item: ${itemType}): Promise<Append${pascalSingular}ItemResult> {\n  try {\n    const redis = await getRedisClient();\n    // Upsert: ensure root array exists\n    await redis.call("JSON.SET", key, "$", "[]", "NX");\n    const res = await redis.call("JSON.ARRAPPEND", key, "$", JSON.stringify(item));\n    const count = Array.isArray(res) && typeof res[0] === "number" ? res[0] : typeof res === "number" ? res : 0;\n    return { success: true, data: count };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_JSON_ARRAPPEND_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        id: `redis-json-arrpop-${tableName}`,
        name: `pop${pascalSingular}Item`,
        kind: "delete",
        description: `Pop item from RedisJSON array [${label}] (JSON.ARRPOP)`,
        signature: `pop${pascalSingular}Item(key: string, index?: number): Promise<Pop${pascalSingular}ItemResult>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "index", type: "number", required: false, defaultValue: "-1" },
        ],
        returnType: `Promise<Pop${pascalSingular}ItemResult>`,
        logicMode: "code",
        code: `export async function pop${pascalSingular}Item(key: string, index: number = -1): Promise<Pop${pascalSingular}ItemResult> {\n  try {\n    const redis = await getRedisClient();\n    const res = await redis.call("JSON.ARRPOP", key, "$", index);\n    if (!res) return { success: true, data: null };\n    const parsed = typeof res === "string" ? JSON.parse(res) : res;\n    const data: ${itemType} | null = Array.isArray(parsed) ? (parsed[0] ?? null) : (parsed ?? null);\n    return { success: true, data };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_JSON_ARRPOP_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        id: `redis-json-getrecent-${tableName}`,
        name: `getRecent${pascalSingular}Items`,
        kind: "findAll",
        description: `Retrieve latest N items from RedisJSON array [${label}] (JSON.GET with slice)`,
        signature: `getRecent${pascalSingular}Items(key: string, count?: number): Promise<GetRecent${pascalSingular}ItemsResult>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "count", type: "number", required: false, defaultValue: "20" },
        ],
        returnType: `Promise<GetRecent${pascalSingular}ItemsResult>`,
        logicMode: "code",
        code: `export async function getRecent${pascalSingular}Items(key: string, count: number = 20): Promise<GetRecent${pascalSingular}ItemsResult> {\n  try {\n    const redis = await getRedisClient();\n    const res = await redis.call("JSON.GET", key, "INDENT", "", "NEWLINE", "", "SPACE", "", "PATH", "$[-" + count + ":]");\n    if (!res || typeof res !== "string") return { success: true, data: [] };\n    const parsed = JSON.parse(res);\n    const data: ${itemType}[] = Array.isArray(parsed) && Array.isArray(parsed[0]) ? parsed[0] : Array.isArray(parsed) ? parsed : [];\n    return { success: true, data };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_JSON_GET_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        id: `redis-json-arrlen-${tableName}`,
        name: `get${pascalSingular}Length`,
        kind: "findById",
        description: `Get array length of RedisJSON [${label}] (JSON.ARRLEN)`,
        signature: `get${pascalSingular}Length(key: string): Promise<Get${pascalSingular}LengthResult>`,
        params: [{ name: "key", type: "string", required: true }],
        returnType: `Promise<Get${pascalSingular}LengthResult>`,
        logicMode: "code",
        code: `export async function get${pascalSingular}Length(key: string): Promise<Get${pascalSingular}LengthResult> {\n  try {\n    const redis = await getRedisClient();\n    const res = await redis.call("JSON.ARRLEN", key, "$");\n    const count = Array.isArray(res) && typeof res[0] === "number" ? res[0] : typeof res === "number" ? res : 0;\n    return { success: true, data: count };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_JSON_ARRLEN_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        id: `redis-json-get-${tableName}`,
        name: `get${pascalSingular}`,
        kind: "findById",
        description: `Retrieve entire RedisJSON array document [${label}] (JSON.GET)`,
        signature: `get${pascalSingular}(key: string): Promise<Get${pascalSingular}Result>`,
        params: [{ name: "key", type: "string", required: true }],
        returnType: `Promise<Get${pascalSingular}Result>`,
        logicMode: "code",
        code: `export async function get${pascalSingular}(key: string): Promise<Get${pascalSingular}Result> {\n  try {\n    const redis = await getRedisClient();\n    const res = await redis.call("JSON.GET", key);\n    if (!res || typeof res !== "string") return { success: true, data: null };\n    const data: ${itemType}[] = JSON.parse(res);\n    return { success: true, data };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_JSON_GET_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        id: `redis-json-set-${tableName}`,
        name: `set${pascalSingular}`,
        kind: "create",
        description: `Set entire RedisJSON array document [${label}] (JSON.SET)`,
        signature: `set${pascalSingular}(key: string, value: ${itemType}[]): Promise<Set${pascalSingular}Result>`,
        params: [
          { name: "key", type: "string", required: true },
          { name: "value", type: `${itemType}[]`, required: true },
        ],
        returnType: `Promise<Set${pascalSingular}Result>`,
        logicMode: "code",
        code: `export async function set${pascalSingular}(key: string, value: ${itemType}[]): Promise<Set${pascalSingular}Result> {\n  try {\n    const redis = await getRedisClient();\n    await redis.call("JSON.SET", key, "$", JSON.stringify(value));\n    return { success: true, data: undefined };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_JSON_SET_ERROR" } };\n  }\n}`,
        enabled: true,
        isAutoGenerated: true,
      },
      {
        ...
        name: `delete${pascalSingular}`,
        kind: "delete",
        description: `Delete RedisJSON document [${label}] (DEL)`,
        signature: `delete${pascalSingular}(key: string): Promise<boolean>`,
        params: [{ name: "key", type: "string", required: true }],
        returnType: "Promise<boolean>",
        logicMode: "code",
  ...
  return [
    {
      ...
      returnType: `Promise<Get${pascalSingular}Result>`,
      logicMode: "code",
      code: `export async function get${pascalSingular}(key: string): Promise<Get${pascalSingular}Result> {\n  try {\n    const redis = await getRedisClient();\n    const data = await redis.get(key);\n    if (!data) return { success: true, data: null };\n    return { success: true, data: JSON.parse(data) };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_GET_ERROR" } };\n  }\n}`,
      enabled: true,
      isAutoGenerated: true,
    },
    {
      ...
      signature: `set${pascalSingular}(key: string, value: Record<string, unknown>, ttlSeconds?: number): Promise<Set${pascalSingular}Result>`,
      params: [
        { name: "key", type: "string", required: true },
        { name: "value", type: "Record<string, unknown>", required: true },
        { name: "ttlSeconds", type: "number", required: false, defaultValue: "3600" },
      ],
      returnType: `Promise<Set${pascalSingular}Result>`,
      logicMode: "code",
      code: `export async function set${pascalSingular}(key: string, value: Record<string, unknown>, ttlSeconds?: number): Promise<Set${pascalSingular}Result> {\n  try {\n    const redis = await getRedisClient();\n    const serialized = typeof value === "string" ? value : JSON.stringify(value);\n    if (ttlSeconds && ttlSeconds > 0) {\n      await redis.setex(key, ttlSeconds, serialized);\n    } else {\n      await redis.set(key, serialized);\n    }\n    return { success: true, data: undefined };\n  } catch (error) {\n    const message = error instanceof Error ? error.message : String(error);\n    return { success: false, error: { message, code: "REDIS_SET_ERROR" } };\n  }\n}`,
      enabled: true,
      isAutoGenerated: true,
    },
    {
...
export function getEntityDbOperations(
  ...
  if (isRedis) {
    ...
    if (entityNode.data.dbOperations && entityNode.data.dbOperations.length > 0) {
      const hasSqlStatements = entityNode.data.dbOperations.some(
        (op) =>
          op.code &&
      ...
      // Check if existing auto-generated operations match the current structure & jsonRootType,
      // and do not contain stale generics (<T) or outdated return types
      const autoGeneratedOps = entityNode.data.dbOperations.filter((op) => op.isAutoGenerated || op.id?.startsWith("redis-"));
      const currentStructure = entityNode.data.redisDataStructure || "hash";
      const isJsonArray = currentStructure === "json" && entityNode.data.jsonRootType === "array";

      let autoOpsMismatch = false;
      if (autoGeneratedOps.length > 0) {
        if (currentStructure === "list") {
          autoOpsMismatch = !autoGeneratedOps.some((op) => op.id.startsWith("redis-rpush-") || op.id.startsWith("redis-lrange-"));
        } else if (isJsonArray) {
          autoOpsMismatch = !autoGeneratedOps.some((op) => op.id.startsWith("redis-json-arrappend-"));
        } else if (currentStructure === "hash") {
          autoOpsMismatch = !autoGeneratedOps.some((op) => op.id.startsWith("redis-hgetall-"));
        } else if (currentStructure === "zset") {
          autoOpsMismatch = !autoGeneratedOps.some((op) => op.id.startsWith("redis-zadd-"));
        } else if (currentStructure === "geo") {
          autoOpsMismatch = !autoGeneratedOps.some((op) => op.id.startsWith("redis-geoadd-"));
        } else if (currentStructure === "stream") {
          autoOpsMismatch = !autoGeneratedOps.some((op) => op.id.startsWith("redis-xadd-"));
        } else if (currentStructure === "json" || currentStructure === "string") {
          autoOpsMismatch = autoGeneratedOps.some((op) =>
            op.id.startsWith("redis-rpush-") ||
            op.id.startsWith("redis-json-arrappend-") ||
            op.id.startsWith("redis-hgetall-") ||
            op.id.startsWith("redis-zadd-") ||
            op.id.startsWith("redis-geoadd-") ||
            op.id.startsWith("redis-xadd-")
          );
        }

        // Detect any stale generic signatures or old non-Result return types in auto-generated operations
        const hasStaleGenericsOrSignatures = autoGeneratedOps.some(
          (op) =>
            op.signature?.includes("<T") ||
            op.code?.includes("<T") ||
            (op.returnType && op.returnType.includes("<T")) ||
            (isJsonArray &&
              (op.name === `get${pascalSingular}` ||
                op.name === `getRecent${pascalSingular}Items` ||
                op.name === `append${pascalSingular}Item` ||
                op.name === `pop${pascalSingular}Item` ||
                op.name === `get${pascalSingular}Length`) &&
              !op.returnType?.includes("Result")) ||
            (currentStructure === "json" &&
              op.name === `get${pascalSingular}` &&
              !op.returnType?.includes("Result")) ||
            (currentStructure === "hash" &&
              (op.name === `getAll${pascalSingular}Fields` ||
                op.name === `get${pascalSingular}Field` ||
                op.name === `set${pascalSingular}Fields`) &&
              !op.returnType?.includes("Result"))
        );
        if (hasStaleGenericsOrSignatures) {
          autoOpsMismatch = true;
        }
      }

      if (!hasSqlStatements && !autoOpsMismatch) {
        return entityNode.data.dbOperations;
      }
      // Cleanse SQL statements or outdated auto-generated ops: preserve only genuine non-SQL custom operations
      const customRedisOps = entityNode.data.dbOperations.filter(
        (op) =>
          !op.isAutoGenerated &&
          !op.id?.startsWith("redis-") &&
          op.code &&
          !op.code.includes("stmtUpdate") &&
          !op.code.includes("stmtInsert") &&
          !op.code.includes("stmtFindAll") &&
      ...
      const redisDefaults = generateRedisOperations(label, entityNode.data);
      const upgradedOps = [...redisDefaults, ...customRedisOps];
