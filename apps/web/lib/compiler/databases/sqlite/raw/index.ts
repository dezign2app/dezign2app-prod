  ...
  dbOps.forEach((op) => {
    ...
    const isLegacyVulnerableCode =
      ...
      (op.code.includes("Object.keys(") ||
        ...
        (pkTs === "number" && (op.code.includes("info.lastInsertRowid.toString()") || op.code.includes("String(info.lastInsertRowid)"))) ||
        (op.kind === "create" && (op.code.includes("as " + Pascal + "Row") || op.code.includes("as " + pascalSingular + "Row") || op.code.includes("as UntitledTableRow")) && !op.code.includes("as unknown as")) ||
    ...
    // Safety transformation: ensure any direct cast "as ...Row" is safe against TS2352
    if (effectiveCode) {
      effectiveCode = effectiveCode.replace(/ as ([A-Za-z0-9_]+Row)(?! \| undefined|\[\]|\s*&)/g, " as unknown as $1");
      effectiveCode = effectiveCode.replace(/ as unknown as unknown as /g, " as unknown as ");
    }
